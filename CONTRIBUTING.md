...
## 🌐 多语言文档维护 / Bilingual docs

- **写作只用中文**：正常写 zh 文档即可，不需要自己写英文版。
- **`.en.*` 文件是机器生成的**：翻译流水线（`pnpm translate:docs`，按段落
  切分 + 翻译记忆增量调用 Claude API）自动生成并保持与中文源同步。
- **欢迎直接修英文**：发现 `.en` 文件翻译不好可以直接改。人工修改受三方
  翻译记忆保护，不会被后续机器翻译覆盖；对翻译文件的人工修复将计入贡献者
  统计（计分规则待维护者批准后生效，机器批量导入和 bot 提交不计）。
- **不想被翻译**：源文件 frontmatter 写 `noTranslate: true` 可整篇跳过。
- **数据披露**：为生成英文版，投稿的文档内容会提交给第三方 LLM API
  （Anthropic）处理。不希望自己的文档参与机器翻译时使用上面的
  `noTranslate` 选项。
