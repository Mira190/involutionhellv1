...
### ✅ P0-2 doc-dates 退出 prebuild —— **已实施（本轮，方案被实测改写）**
...
- **证据**：本轮实测 + remediation 分支上的 1 条日期漂移（08-03→08-06，
  由前一个 commit 触碰该文件所致）。
- **层次**：构建产物治理层。
- **实测推翻了原方案，并暴露一个生产级正确性 bug**：把仓库浅克隆
  （`git clone --depth=1`）后跑生成器，**423 篇文档全部拿到同一个日期**
  （= checkout 当天）。Vercel 构建默认就是浅克隆，而该生成器挂在
  `prebuild` 上——也就是说线上每次部署都会用一份"全站同一天更新"的坏数据
  覆盖仓库里的好数据，页面显示的"最后更新"系统性错误且无人察觉。
  本沙盒仓库本身也是浅克隆（有 `.git/shallow`，可见 163 个 commit），
  说明**已 commit 的这份数据本身也是部分错误的**。
- **改用的方案**：
  1. 生成器加浅克隆断言——检测到 `is-shallow-repository=true` 直接
     退出并说明原因，坏数据永远不可能再被写出；
  2. 从 `prebuild` 摘除（build 不再重生成）→ 文件成为纯 committed 数据；
  3. 新增 `.github/workflows/refresh-doc-dates.yml`（weekly + 手动，
     `fetch-depth: 0`）负责刷新并 commit；
  4. 两处 CI 的 `doc-dates.json` 豁免**全部删除**，恢复无豁免的全量检查。
- **实测度量**：修前 build 后工作树脏 2 个文件，修后 **0**（豁免已删仍通过）。
- **遗留**：仓库里现存的 doc-dates 数据需由 refresh workflow 首次运行
  （完整历史）刷新一次才完全正确。
